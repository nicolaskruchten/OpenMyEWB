...
import ca.myewb.frame.forms.GroupDirectoryCreationForm;
import ca.myewb.model.GroupModel;


public class SaveGroupDirectory extends Controller
{
	public void handle(Context ctx) throws Exception
	{
		...
		log.info("Path set at " + filePath);
		
		// Create & validate form object
		GroupDirectoryCreationForm form = new GroupDirectoryCreationForm(path + "/actions/SaveGroupFilesFile/" + g.getId() + "/" + filePath, requestParams);
		...
		if(requestParams.get("folder") != null){
			
			log.info("Folder to create is " + requestParams.get("folder"));
			
			requestParams.createDirectory(requestParams.get("folder"), filePath);
		}
				
		// Leave a message in the session
		setSessionMessage(("Your folder was successfully created."));
